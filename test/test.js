...
describe("Artist.sol", function () {
    ...
    beforeEach(async function () {
        ...
        const ArtistTx = await MCFactory.connect(addr1).createArtist(addr1.address, "Artist", "IMG", "DESC", "Social"); //Create Artist contract instances
        // Functions that write data to the blockchain don't return any values. They return the transaction hash
        await ArtistTx.wait();
        artistList = await MCFactory.getArtistList();
        // It should be possible to read the contract address from the transaction receipt...
        // console.log('TX Hash:', await ArtistTx.events);
        
        
        Artist = await ArtistContract.attach(artistList[0]);
        
        await MusicoinToken.transfer(addr1.address, 100);
        await MusicoinToken.transfer(addr2.address, 100);
        console.log('Balance: ', (await MusicoinToken.balanceOf(addr1.address)).toNumber());
  ...
  //      console.log('Artist owner: ', await Artist.owner());
        it("should deploy artist to the correct owner", async function () {
            expect(await Artist.owner()).to.equal(addr1.address);
        });

        // Musicoin allocated correctly
//
//        it("should deploy artist to the correct owner", async function () {
  //          expect(await MusicoinToken.balanceOf(addr1.address), 10)
//
  //      });
 ...
 //+++++++++*******       console.log('Balance: ', (await MusicoinToken.balanceOf(addr1.address)).toNumber());
  //      it("transfer test", async function () {
    //        const total = await MusicoinToken.balanceOf(addr1.address);
      //      expect(await MusicoinToken.balanceOf(addr1.address)).to.be.a.bignumber.that.equals(addr1.address);
    ...
    describe("Artist.sol Interactive Tests", async function () {
        ...
        describe("Testing TIPPING function", async function () {
            ...
            it("should tip the artist 2 Musicoin", async function () {
                ...
                console.log('MF: ', MCFactory.address);
                console.log('A.MF: ', Artist.address);
		        await MusicoinToken.connect(addr2).approve(MCFactory.address, 100);
